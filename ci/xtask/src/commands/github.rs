...
#[cfg(test)]
mod tests {
    use {super::*, pretty_assertions::assert_eq, std::sync::Mutex};

    // Serialise tests that touch BUILDKITE_REPO so they don't race each other.
    static ENV_LOCK: Mutex<()> = Mutex::new(());

    // Restore BUILDKITE_REPO to its prior state (removing it if it was unset).
    fn restore_buildkite_repo(prev: Option<String>) {
        match prev {
            Some(val) => unsafe { env::set_var("BUILDKITE_REPO", val) },
            None => unsafe { env::remove_var("BUILDKITE_REPO") },
        }
    }
    ...
    fn test_from_env_falls_back_to_default_when_unset() {
        ...
        let prev = env::var("BUILDKITE_REPO").ok();
        unsafe { env::remove_var("BUILDKITE_REPO") };
        let repo = Repo::from_env();
        restore_buildkite_repo(prev);
        assert_eq!(repo.owner, "anza-xyz");
        assert_eq!(repo.name, "agave");
    }

    #[test]
    fn test_from_env_parses_buildkite_repo_https() {
        let _guard = ENV_LOCK.lock().unwrap();
        let prev = env::var("BUILDKITE_REPO").ok();
        unsafe { env::set_var("BUILDKITE_REPO", "https://github.com/my-org/my-repo.git") };
        let repo = Repo::from_env();
        restore_buildkite_repo(prev);
    ...
    #[test]
    fn test_from_env_parses_buildkite_repo_ssh() {
        let _guard = ENV_LOCK.lock().unwrap();
        let prev = env::var("BUILDKITE_REPO").ok();
        unsafe { env::set_var("BUILDKITE_REPO", "git@github.com:my-org/my-repo.git") };
        let repo = Repo::from_env();
        restore_buildkite_repo(prev);
        assert_eq!(repo.owner, "my-org");
        assert_eq!(repo.name, "my-repo");
    ...
    #[test]
    fn test_from_env_falls_back_when_url_is_unparseable() {
        let _guard = ENV_LOCK.lock().unwrap();
        let prev = env::var("BUILDKITE_REPO").ok();
        unsafe { env::set_var("BUILDKITE_REPO", "not-a-url") };
        let repo = Repo::from_env();
        restore_buildkite_repo(prev);
        assert_eq!(repo.owner, "anza-xyz");
        assert_eq!(repo.name, "agave");
