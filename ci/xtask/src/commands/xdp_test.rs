...
fn run_vm(args: VmArgs, release: bool) -> Result<()> {
    ...
    let downloaded_archives = resolve_kernel_set_archives(&cache_dir, &args.kernel_sets)?;
    for archive in downloaded_archives {
        kernels.push(KernelSource::Archive(archive));
    }
...
fn fetch_kernels(args: FetchKernelsArgs) -> Result<()> {
    ...
    fs::create_dir_all(&cache_dir)
        .with_context(|| format!("failed to create {}", cache_dir.display()))?;
    let archives = resolve_kernel_set_archives(&cache_dir, &args.kernel_sets)?;
...
fn resolve_kernel_set_archives(
    cache_dir: &Path,
    ...
    for version in versions {
        let url = select_kernel_url(&index, "amd64", version)
...
fn kernel_versions(kernel_set: KernelSet) -> &'static [&'static str] {
    match kernel_set {
        KernelSet::Pr => &["6.1", "6.12"],
        KernelSet::Nightly => &["5.10", "6.1", "6.12", "6.17"],
    }
}

fn fetch_debian_kernel_index() -> Result<Vec<String>> {
    const MIRROR: &str = "https://mirrors.wikimedia.org/debian/pool/main/l/linux/";
    let output = Command::new("curl")
        .args(["-fsSL", MIRROR])
        .output()
        .with_context(|| format!("failed to fetch {MIRROR}"))?;
    ensure!(
        output.status.success(),
        "curl failed for {MIRROR} with {}",
        output.status
    );

    let html = String::from_utf8(output.stdout).context("Debian index is not valid UTF-8")?;
    ...
    let urls = href
        ...
        .collect::<Vec<_>>();
    ensure!(!urls.is_empty(), "no .deb links found in {MIRROR}");
    Ok(urls)
}

fn select_kernel_url(index: &[String], arch: &str, version: &str) -> Result<String> {
    let version = regex::escape(version);
    let arch = regex::escape(arch);
    let pattern = format!(
        r"linux-image-{version}\.[0-9]+(-[0-9]+)?(\+bpo|\+deb[0-9]+)?-cloud-{arch}-unsigned_.*\.deb$"
    );
