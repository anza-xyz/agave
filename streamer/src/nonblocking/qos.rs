...
pub(crate) trait OpaqueStreamerCounter: Send + Sync {
    fn as_any_arc(self: Arc<Self>) -> Arc<dyn Any + Send + Sync>;
}

/// Helper to downcast OpaqueStreamerCounter into specific types
pub(crate) fn get_shared_state<T: OpaqueStreamerCounter + 'static>(
    x: Arc<dyn OpaqueStreamerCounter>,
) -> Option<Arc<T>> {
