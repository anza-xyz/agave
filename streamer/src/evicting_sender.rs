...
///
/// Generic over the two channel ends; the defaults are crossbeam's, and `agave_wake_channel`
...
#[cfg(test)]
mod tests {
    use {super::*, agave_wake_channel::WakeGroup, std::sync::Arc};

    #[test]
    fn test_evicts_oldest_on_wake_lane() {
        let (sender, receiver) = agave_wake_channel::bounded(2, Arc::new(WakeGroup::default()));
