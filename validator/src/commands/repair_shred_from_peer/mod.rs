...
pub struct RepairShredFromPeerArg {
    pub pubkey: Option<Pubkey>,
    pub slot: Option<u64>,
    pub shred: Option<u64>,
}

impl FromClapArgMatches for RepairShredFromPeerArg {
...
pub fn execute(matches: &ArgMatches, ledger_path: &Path) {
    let repair_shred_from_peer_arg = RepairShredFromPeerArg::from_clap_arg_match(matches);
    if repair_shred_from_peer_arg.slot.is_none() {
        eprintln!("slot is required");
        exit(1);
    }
    if repair_shred_from_peer_arg.shred.is_none() {
        eprintln!("shred is required");
        exit(1);
    }

    let pubkey = repair_shred_from_peer_arg.pubkey;
    let slot = repair_shred_from_peer_arg.slot.unwrap();
    let shred = repair_shred_from_peer_arg.shred.unwrap();
...
mod tests {
    ...
    fn verify_args_struct_by_command_repair_shred_from_peer_default() {
        ...
        let arg = RepairShredFromPeerArg::from_clap_arg_match(&matches);
        assert_eq!(
            arg,
            RepairShredFromPeerArg {
                pubkey: None,
                slot: None,
                shred: None,
            },
