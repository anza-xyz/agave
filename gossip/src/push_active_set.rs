...
mod tests {
    ...
    fn test_push_active_set() {
        ...
        let stakes = repeat_with(|| rng.random_range(1..MAX_STAKE));
        let mut stakes: HashMap<_, _> = nodes.iter().copied().zip(stakes).collect();
        stakes.insert(pubkey, rng.random_range(1..MAX_STAKE));
    ...
    fn test_push_active_set_entry() {
        ...
        let nodes: Vec<_> = repeat_with(Pubkey::new_unique).take(20).collect();
        let weights: Vec<_> = repeat_with(|| rng.random_range(1..1000)).take(20).collect();
