...
use {
    ...
    solana_net_utils::{
        ...
        bind_to_with_config, bind_two_in_range_with_offset_and_config, get_gossip_port,
...
impl Node {
    ...
    pub fn new_localhost_with_pubkey(pubkey: &Pubkey) -> Self {
        ...
        Self::new_single_bind(pubkey, &gossip_addr, VALIDATOR_PORT_RANGE, addr)
    }

    /// new_single_bind will be removed soon
    pub fn new_single_bind(
        ...
        let (rpc_port, socket1) = bind_in_range(bind_ip_addr, port_range).expect("RPC");
        let (rpc_pubsub_port, socket2) =
            bind_in_range(bind_ip_addr, port_range).expect("RPC pubsub");
        // immediately close both sockets so other code can actually bind them later
        drop((socket1, socket2));
    ...
    pub fn new_with_external_ip(pubkey: &Pubkey, config: NodeConfig) -> Node {
        ...
        let tpu_forwards_sockets = bind_more_with_config(tpu_forwards_sockets, 8, socket_config)
