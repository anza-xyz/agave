    ...
    fn try_send(&self, msg: T) -> std::result::Result<(), TrySendError<T>> {
        ...
        match e {
            ...
            TrySendError::Full(msg) => match self.receiver.try_recv() {
                ...
                // Propagate the error rather than looping to avoid blocking in degenerate cases.
                Err(TryRecvError::Empty) => Err(TrySendError::Full(msg)),
                // Unreachable in practice since we maintain a reference to both the sender and receiver.
                Err(TryRecvError::Disconnected) => Err(TrySendError::Disconnected(msg)),
            },
            // Unreachable in practice since we maintain a reference to both the sender and receiver.
