...
mod tests {
    ...
    fn test_discard_different_shred_version_push_message() {
        ...
        if let Protocol::PushMessage(_, ref values) = msg {
            // Fail to insert since EpochSlot is not associated with any ContactInfo already in the table
            assert_eq!(values.len(), 0);
        }
        ...
        // Test insert EpochSlot w/ previous ContactInfo w/ matching shred version but differentpubkey -> should be rejected
        let epoch_slots = EpochSlots::new_rand(&mut rng, Some(keypair.pubkey()));
        ...
        if let Protocol::PushMessage(_, ref values) = msg {
            // Fail to insert since EpochSlot is associated with different pubkey
            assert_eq!(values.len(), 0);
        }
        ...
        if let Protocol::PushMessage(_, ref values) = msg {
            // Successfully insert EpochSlotsince ContactInfo is associated with matching pubkey
            assert_eq!(values.len(), 1);
        }
