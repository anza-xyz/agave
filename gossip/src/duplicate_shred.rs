...
pub(crate) fn into_shreds(
    ...
    if enforce_correct_proof_size {
        for shred in [&shred1, &shred2] {
            let proof_size = shred.proof_size().unwrap_or(0);
            if !shred::has_correct_proof_size(proof_size) {
...
pub(crate) mod tests {
    ...
    #[test]
    fn test_into_shreds_rejects_bad_proof_size() {
        // Offset of ShredCommonHeader.shred_variant, whose low nibble is the
        // Merkle proof height.
        const OFFSET_OF_SHRED_VARIANT: usize = std::mem::size_of::<Signature>();
        let mut rng = rand::rng();
        let leader = Arc::new(Keypair::new());
        let (slot, parent_slot, reference_tick, version) = (53084024, 53084023, 0, 0);
        let shredder = Shredder::new(slot, parent_slot, reference_tick, version).unwrap();
        ...
        let shred2 = {
            ...
            let shred_variant = payload[OFFSET_OF_SHRED_VARIANT];
            assert_eq!(
                shred_variant & 0x0F,
                shred2.proof_size().unwrap(),
                "low nibble of shred_variant holds the proof height"
            );
            payload[OFFSET_OF_SHRED_VARIANT] = (shred_variant & 0xF0) | 5;
            Shred::new_from_serialized_shred(payload).unwrap()
        };
        assert_eq!(shred2.proof_size().unwrap(), 5);
        ...
        assert_matches!(
            ...
            Err(Error::InvalidShred(shred::Error::InvalidProofSize(5)))
        );
