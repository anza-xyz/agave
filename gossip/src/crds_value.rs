...
/// Number of votes per validator to store. While only the latest
/// vote is needed for liveness, we add a buffer to allow for better
/// user experience by giving each vote a fair chance to be observed.
pub const MAX_VOTES: VoteIndex = 6;
