...
impl Node {
    ...
    pub fn new_with_external_ip(pubkey: &Pubkey, config: NodeConfig) -> Node {
        ...
        let (tpu_port, tpu_socket) =
            bind_in_range_with_config(bind_ip_addr, port_range, socket_configs.primarily_read_udp)
                .expect("tpu_socket primary bind");
        let tpu_sockets = bind_more_with_config(tpu_socket, 32, socket_configs.primarily_read_udp)
        ...
        let tpu_forwards_sockets =
            bind_more_with_config(tpu_forwards_socket, 8, socket_configs.primarily_read_udp)
