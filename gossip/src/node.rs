...
impl Node {
    ...
    pub fn new_with_external_ip(pubkey: &Pubkey, config: NodeConfig) -> Node {
        ...
        bind_ip_addrs.iter().map(
            |addr |{let (_, socket) =
            bind_in_range_with_config(bind_ip_addr, port_range, socket_config).expect("TPU transaction forwarding client bind on interface {bind_ip_addr} should succeed" );
            socket
            }).collect();
        ...
        let sockets = Sockets {
            ...
            quic_vote_client,
            tpu_transaction_forwarding_clients: tpu_transaction_forwarding_client,
            rpc_sts_client,
            vortexor_receivers,
        };
