use {
    ...
    tokio::{runtime::Handle, sync::mpsc, time::sleep},
    tokio_serde::formats::Bincode,
    tokio_util::sync::CancellationToken,
};

const TRANSACTION_CHANNEL_SIZE: usize = 1_000;
...
impl BanksServer {
    ...
    fn run(
        bank_forks: Arc<RwLock<BankForks>>,
        mut transaction_receiver: mpsc::Receiver<TransactionInfo>,
    ) {
        while let Some(info) = transaction_receiver.blocking_recv() {
...
impl Banks for BanksServer {
    async fn send_transaction_with_context(self, _: Context, transaction: VersionedTransaction) {
        ...
        self.transaction_sender
            .try_send(info)
            .unwrap_or_else(|err| warn!("Failed to send transaction: {err}"));
    }
...
pub async fn start_tcp_server(
    ...
    let server = tcp::listen(listen_addr, Bincode::default)
        ...
        .map(move |chan| {
            let (sender, receiver) = mpsc::channel(TRANSACTION_CHANNEL_SIZE);
