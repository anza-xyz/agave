...
impl BankingStage {
    ...
    pub fn spawn_non_vote_threads(
        ...
        if let Some(context) = self.non_vote_context.as_ref() {
            ...
            info!(
                "Spawning new banking stage non-vote threads with block-production-method: {block_production_method:?} \
                transaction-structure: {transaction_struct:?} num-workers: {num_workers}"
            );
