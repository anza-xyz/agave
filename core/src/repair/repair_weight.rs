...
impl RepairWeight {
    ...
    pub fn set_root(&mut self, new_root: Slot) {
        ...
        let new_root_tree_root = match self.slot_to_tree.get(&new_root).copied() {
            Some(TreeRoot::Root(r)) => Some(r),
            Some(TreeRoot::PrunedRoot(r)) => {
                warn!(
                    "New root {new_root} chains to a pruned tree with root {r}; treating repair \
                     weight pruned state as stale"
                );
                match self.pruned_trees.remove(&r) {
                    Some(pruned_tree) => {
                        if let Some(replaced_tree) = self.trees.remove(&r) {
                            warn!(
                                "Replacing existing repair tree rooted at {r} with stale pruned \
                                 tree"
                            );
                            self.remove_tree_slots(&replaced_tree);
                        }
                        self.rename_tree_root(&pruned_tree, TreeRoot::Root(r));
                        self.trees.insert(r, pruned_tree);
                        Some(r)
                    }
                    None => {
                        warn!("Missing repair pruned tree rooted at {r}; dropping stale mappings");
                        self.slot_to_tree
                            .retain(|_, tree_root| *tree_root != TreeRoot::PrunedRoot(r));
                        None
                    }
                }
            }
...
mod test {
    ...
    #[test]
    fn test_set_root_to_pruned_tree_treats_pruned_state_as_stale() {
        let blockstore = setup_big_forks();
        let stake = 100;
        let (bank, vote_pubkeys) = bank_utils::setup_bank_and_vote_pubkeys_for_tests(3, stake);
        let votes = vec![
        ...
        repair_weight.set_root(2);
        assert_eq!(
            *repair_weight.slot_to_tree.get(&3).unwrap(),
            TreeRoot::PrunedRoot(3)
        );

        repair_weight.set_root(3);
        assert_eq!(repair_weight.root, 3);
        assert_eq!(
            *repair_weight.slot_to_tree.get(&3).unwrap(),
            TreeRoot::Root(3)
        );
        assert!(repair_weight.trees.contains_key(&3));
        assert!(!repair_weight.pruned_trees.contains_key(&3));
    }
