...
use {
    ...
    solana_perf::{
        ...
        packet::{BytesPacket, Packet, PacketBatch, PacketBatchRecycler, RecycledPacketBatch},
    },
...
impl ServeRepair {
    ...
    fn run_listen(
        ...
        loop {
            let Ok(mut more) = requests_receiver.try_recv() else {
                break;
            };
            let more: Vec<_> = more.into_iter().map(|p| p.to_bytes_packet()).collect();
            ...
            if retained > 0 && well_formed_requests <= max_buffered_packets {
                requests.extend(more.into_iter().map(|p| p.to_bytes_packet()));
            } else {
                dropped_requests += more.len();
            }
        }
