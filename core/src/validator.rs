...
pub struct ValidatorConfig {
    ...
    /// inbound connections and pushing consensus messages to them. `None` resolves the
    /// address from gossip (what `--votor-forwards` produces), `Some` pins it.
    /// Empty = only the staked set participates.
