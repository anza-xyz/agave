...
mod tests {
    ...
    fn send_transaction_packet_batches<T>(sender: &Sender<BankingPacketBatch>, items: &[T])
    where
    ...
    fn test_receive_and_buffer_no_hold() {
        ...
        let packet_batches = banking_packet_batch_from_transaction(&transaction);
    ...
    fn test_receive_and_buffer_discard() {
        ...
        let mut packet_batches = banking_packet_batch_from_transaction(&transaction);
    ...
    fn test_receive_and_buffer_invalid_transaction_format() {
        ...
        let packet_batches = banking_packet_batch_from_bytes(vec![1u8; PACKET_DATA_SIZE]);
        sender.send(packet_batches).unwrap();
    ...
    fn test_receive_and_buffer_overfull() {
        ...
        send_transaction_packet_batches(&sender, &transactions);
