...
impl<Tx: TransactionWithMeta> Scheduler<Tx> for PrioGraphScheduler<Tx> {
    ...
    fn schedule<S: StateContainer<Tx>>(
        ...
        while let Some((id, _)) = self.prio_graph.pop_and_unblock() {
            container.put_id_back_into_queue(id);
        }
...
mod tests {
    ...
    fn test_schedulling(
        ...
        let post_schedule_remaining_capacity = container.remaining_capacity();
        let mut post_schedule_remaining_ids = 0;
        while let Some(_p) = container.pop() {
            post_schedule_remaining_ids += 1;
        }

        // This test never receives finished_work, the capacity shall remain 0 after scheduling
        assert_eq!(post_schedule_remaining_capacity, 0);
