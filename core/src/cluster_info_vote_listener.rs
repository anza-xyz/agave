...
mod tests {
    ...
    fn run_test_process_votes(hash: Option<Hash>) {
        ...
        for (new_slot, received_pubkeys) in verified_voter_slots_receiver.try_iter() {
            ...
            let already_received_slots = pubkey_to_slots.entry(received_pubkeys[0]).or_default();
            ...
            .enumerate()
        {
            let validator_votes: Vec<_> = keyset
                ...
                .map(|keypairs| {
                    let node_keypair = &keypairs.node_keypair;
                    let vote_keypair = &keypairs.vote_keypair;
                    expected_voter_slots.push((i as Slot + 1, vec![vote_keypair.pubkey()]));
                    let tower_sync =
