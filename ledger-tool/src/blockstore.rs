        ...
        SubCommand::with_name("copy")
            ...
            .arg(
                Arg::with_name("output_dir")
                    ...
                    .value_name("DIR")
                    .takes_value(true)
                    .help("Destination directory to write inner \"rocksdb\" within."),
            ),
...
fn do_blockstore_process_command(ledger_path: &Path, matches: &ArgMatches<'_>) -> Result<()> {
    ...
    match matches.subcommand() {
        ...
        ("copy", Some(arg_matches)) => {
            ...
            let output_dir = PathBuf::from(value_t_or_exit!(arg_matches, "output_dir", String));

            let source = crate::open_blockstore(&ledger_path, arg_matches, AccessType::Secondary);
            let target =
                crate::open_blockstore(&output_dir, arg_matches, AccessType::PrimaryForMaintenance);
