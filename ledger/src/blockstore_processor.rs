...
fn do_get_first_error<T, Tx: SVMTransaction>(
    ...
    for (result, transaction) in results.iter().zip(batch.sanitized_transactions()) {
        if let Err(err) = result {
            ...
            datapoint_error!(
                solana_metrics::names::ledger::VALIDATOR_PROCESS_ENTRY_ERROR,
                (
                    "error",
...
pub fn process_blockstore_from_root(
    ...
    datapoint_info!(
        solana_metrics::names::ledger::PROCESS_BLOCKSTORE_FROM_ROOT,
...
impl ThreadExecuteTimings {
    pub fn report_stats(&self, slot: Slot) {
        lazy! {
            datapoint_info!(
                solana_metrics::names::ledger::REPLAY_SLOT_END_TO_END_STATS,
...
impl ReplaySlotStats {
    pub fn report_stats(
        ...
        lazy! {
            datapoint_info!(
                solana_metrics::names::ledger::REPLAY_SLOT_STATS,
                ("slot", slot as i64, i64),
        ...
        if log::log_enabled!(log::Level::Trace) {
            ...
            for (pubkey, time) in per_pubkey_timings.iter().take(5) {
                datapoint_trace!(
                    solana_metrics::names::ledger::PER_PROGRAM_TIMINGS,
                    ("slot", slot as i64, i64),
...
fn confirm_slot_entries(
    ...
    if !skip_verification {
        ...
        progress.async_verification().spawn(
            ...
            move || {
                datapoint_debug!(
                    solana_metrics::names::ledger::VERIFY_BATCH_SIZE,
