...
fn convert_update_parent_to_block_header(marker: VersionedBlockMarker) -> VersionedBlockMarker {
...
) -> (Vec<EntrySummary>, Vec<VersionedBlockMarker>) {
        ...
        match component {
            ...
            ConfirmedBlockComponent::BlockMarker(marker) => {
                if marker.is_parent_marker() {
                    ...
                    debug_assert!(
                        !seen_parent_marker,
                        "rooted block data should only contain one parent marker"
                    );
                    seen_parent_marker = true;
                }

                let marker = convert_update_parent_to_block_header(marker);
                markers.push(marker);
            }
        }
    }
