...
impl Blockstore {
    ...
    /// Removes transaction history that may be incomplete or disagree with a bank restored from a
    /// snapshot.
    ///
    /// The safe root records the last canonical root processed by TransactionStatusService. Slots
    /// after it may contain only part of their transaction history when the validator exits, while
    /// the snapshot causes those slots to be skipped during replay. When upgrading a ledger that
    /// has no safe root yet, only UpdateParent slots are purged to preserve existing history.
    pub fn reconcile_transaction_history_for_snapshot_slots(
        &self,
        snapshot_slot: Slot,
    ) -> Result<()> {
            ...
            .take_while(|(slot, _)| *slot <= snapshot_slot)
        {
            ...
            if is_canonical && (has_safe_root || meta.has_update_parent()) {
                slots_to_purge.push(slot);
            }
        }
        ...
        datapoint_info!(
            ...
            ("slots_purged", slots_to_purge.len(), i64),
        );
        Ok(())
    }
...
pub mod tests {
    use {
        ...
        crate::{
            blockstore::tests::{
                ...
                write_transaction_statuses_for_entries,
            },
    ...
    fn test_purge_transaction_history_exact_leader_uses_transactions() {
        ...
        let signatures = write_transaction_statuses_for_entries(&blockstore, slot, &entries);
    ...
    #[test]
    fn test_purge_transaction_history_exact_uses_update_parent_marker() {
        let ledger_path = get_tmp_ledger_path_auto_delete!();
        let blockstore = Blockstore::open(ledger_path.path()).unwrap();
        let slot = 104;
        ...
        assert_eq!(
            stats.transactions_processed,
            fixture.pre_update_signatures.len() as u64
        );

        let pre_update_signature = fixture.pre_update_signatures[0];
        assert!(
            blockstore
        ...
        assert!(
            blockstore
                .address_signatures_cf
                .get((fixture.pre_update_address, slot, 0, pre_update_signature))
                .unwrap()
                .is_none()
        );

        let post_update_signature = fixture.post_update_signatures[0];
        assert!(
            blockstore
                .read_transaction_status((post_update_signature, slot))
                .unwrap()
                .is_some()
        );
        assert!(
            blockstore
                .read_transaction_memos(post_update_signature, slot)
                .unwrap()
                .is_some()
        );
        assert!(
    ...
    #[test]
    fn test_reconcile_transaction_history_for_snapshot_slots() {
        let ledger_path = get_tmp_ledger_path_auto_delete!();
        let blockstore = Blockstore::open(ledger_path.path()).unwrap();
        ...
        let next_rooted_slot = snapshot_slot + 1;
        let next_snapshot_slot = next_rooted_slot + 1;
        let insert_slot_with_transaction_history = |slot, parent| {
            let entries = make_slot_entries_with_transactions(1);
            blockstore
                .insert_shreds(
                    entries_to_test_shreds(&entries, slot, parent, true, 0),
                    true,
                )
                .unwrap();
            write_transaction_statuses_for_entries(&blockstore, slot, &entries)[0]
        };
        let rooted_signature =
            insert_slot_with_transaction_history(next_rooted_slot, snapshot_slot);
        blockstore
            .set_roots(std::iter::once(&next_rooted_slot))
            .unwrap();
        let next_snapshot_signature =
            insert_slot_with_transaction_history(next_snapshot_slot, next_rooted_slot);

        blockstore
            .reconcile_transaction_history_for_snapshot_slots(next_snapshot_slot)
            .unwrap();

        for (signature, slot) in [
            (rooted_signature, next_rooted_slot),
            (next_snapshot_signature, next_snapshot_slot),
        ] {
            assert!(
                blockstore
                    .read_transaction_status((signature, slot))
