...
* Added Linux-only `--config <PATH>` for TOML-based validator settings, initially covering XDP and PoH CPU placement. CLI flags override file values. `[interfaces.<name>]` sections configure a NIC's hardware queue -> CPU worker assignment and zero-copy; `[xdp.<name>]` sections declare an XDP endpoint and the interface it reads from, sharing that interface's worker pool with any other endpoint naming it (only one endpoint is supported today). XDP and PoH CPU assignments may not overlap; other managed threads can opt into overlap checks with `reservation = "exclusive"`.
### CLI
#### Breaking
#### Changes
