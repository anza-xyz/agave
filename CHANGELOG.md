...
* Added Linux-only `--config <PATH>` for TOML-based validator settings, initially covering XDP and PoH CPU placement. CLI flags override file values. `[interfaces.<name>]` sections attach settings (zero-copy) to a NIC; `[xdp.<name>]` sections declare an XDP endpoint and the interface it transmits over (only one endpoint is supported today). XDP and PoH CPU assignments may not overlap; other managed threads can opt into overlap checks with `reservation = "exclusive"`.
### CLI
#### Breaking
#### Changes
