...
impl<'s, 'r, R: RequiredLenBufFileRead<'s>> AccountStorageReader<'s, 'r, R> {
    ...
    pub fn new(
        ...
        Ok(Self {
            storage,
            reader: file_reader,
            sorted_excluded_offsets: excluded_accounts
                .into_iter()
                .map(|(offset, _)| offset)
                .collect(),
            len_for_archive,
    ...
    pub fn write_to(mut self, output: &mut dyn Write) -> io::Result<()> {
        ...
        let scan_result =
            self.storage
                ...
                .scan_accounts_with(self.reader, |offset, account| {
                    ...
                    if self.sorted_excluded_offsets.last() == Some(&offset) {
                        self.sorted_excluded_offsets.pop();
                        return;
                    }
                    ...
                    write_result = write_append_vec_account(output, &account);
...
/// Writes an account in AppendVec format, independent of its source storage format.
fn write_append_vec_account(
    output: &mut dyn Write,
    account: &StoredAccountInfo<'_>,
) -> io::Result<()> {
    ...
    let mut header = [0u8; STORE_META_OVERHEAD];

    let data_len = mem::offset_of!(StoredMeta, data_len);
    header[data_len..data_len + 8].copy_from_slice(&(account.data.len() as u64).to_le_bytes());

    let pubkey = mem::offset_of!(StoredMeta, pubkey);
    header[pubkey..pubkey + 32].copy_from_slice(account.pubkey.as_ref());

    let meta = size_of::<StoredMeta>();

    let lamports = meta + mem::offset_of!(AccountMeta, lamports);
    header[lamports..lamports + 8].copy_from_slice(&account.lamports.to_le_bytes());

    let rent_epoch = meta + mem::offset_of!(AccountMeta, rent_epoch);
    header[rent_epoch..rent_epoch + 8].copy_from_slice(&account.rent_epoch.to_le_bytes());

    let owner = meta + mem::offset_of!(AccountMeta, owner);
    header[owner..owner + 32].copy_from_slice(account.owner.as_ref());

    let executable = meta + mem::offset_of!(AccountMeta, executable);
    header[executable] = u8::from(account.executable);

    output.write_all(&header)?;
    output.write_all(account.data)?;

    // all accounts in an append vec are padded for alignment
    output.write_all(&[0; 7][..stored_size - unaligned_stored_size])
}
...
mod tests {
    use {
        ...
        agave_fs::{buffered_reader::FileBufRead as _, io_setup::IoSetupState},
        log::*,
        rand::{
            SeedableRng,
            rngs::StdRng,
    ...
    fn test_account_storage_reader_with_excluded_accounts(
        ...
        if (total_accounts - number_of_accounts_to_remove) != 0 {
            let (accounts_file, num_accounts) =
                AccountsFile::new_from_file(temp_file_path, reader_len).unwrap();
    ...
    fn test_write_to(exclude_last_account: bool, data_len_last_account: usize) {
        ...
        let (archived_storage, archived_num_accounts) =
            AccountsFile::new_from_file(archive_path, archive_len).unwrap();
