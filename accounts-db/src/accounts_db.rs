...
impl AccountsDb {
    ...
    pub fn generate_index(
        ...
        let mut total_zero_lamport_pubkeys = Vec::new();
        ...
        thread::scope(|s| {
                ...
                .map(|i| {
                    thread::Builder::new()
                        ...
                        .spawn_scoped(s, || {
                            ...
                            let mut thread_lt_hash = LtHash::identity();
                            let mut reader = append_vec::new_scan_accounts_reader();
                            while let Some(storage) = storages_orderer.next() {
                                let store_id = storage.id();
                                let slot = storage.slot();
                                ...
                                if all_accounts_are_zero_lamports {
                                    ...
                                    thread_all_zeros_slots.push((slot, Arc::clone(storage)));
                                }
            ...
            for thread_handle in thread_handles {
                ...
                let (
                    ...
                    thread_num_accounts,
                    thread_accounts_data_len,
                    mut thread_zero_lamport_pubkeys,
                    thread_all_accounts_are_zero_lamports_slots,
                ...
                total_zero_lamport_pubkeys.append(&mut thread_zero_lamport_pubkeys);
    ...
    fn visit_zero_lamport_pubkeys_during_startup(&self, pubkeys: &[Pubkey]) -> u64 {
        let mut count = 0;
        self.accounts_index.scan(
            pubkeys.iter(),
            |_pubkey, slots_refs, _entry| {
