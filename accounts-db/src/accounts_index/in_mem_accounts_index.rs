...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> InMemAccountsIndex<T, U> {
    ...
    fn gather_possible_evictions<'a>(
        ...
        if let Some(max) = max_evictions {
            ...
            if estimated_total_entries_exclude_skipped_ages == 0 || max == 0 {
                return possible_evictions;
            }

            let mut rng = thread_rng();
            let mut remaining_to_select = max.min(estimated_total_entries_exclude_skipped_ages);
            let mut remaining_entries = estimated_total_entries_exclude_skipped_ages;
