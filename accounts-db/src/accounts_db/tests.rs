...
fn run_test_remove_unrooted_slot(is_cached: bool, db: AccountsDb) {
    ...
    if is_cached {
        ...
        // will be fixed next... currently stores to cache also...
        db.store_for_tests(unrooted_slot, &[(&key, &account0)]);
    }
    assert!(db.accounts_index.contains(&key));
...
fn test_account_matches_owners() {
    ...
    db.store_for_tests(0, &[(&account1_key, &account1)]);
    db.store_for_tests(1, &[(&account2_key, &account2)]);
    db.store_for_tests(2, &[(&account3_key, &account3)]);
    db.store_for_tests(3, &[(&account4_key, &account4)]);
...
fn test_flush_cache_populates_uncleaned_pubkeys() {
    ...
    accounts_db.store_for_tests(slot, &[(&pubkey, &account)]);
