...
use {
    ...
    crate::{buffered_reader::FileBufRead, io_uring::sqpoll},
...
impl<'sp> SequentialFileReaderBuilder<'sp> {
    ...
    pub fn build_with_buffer<'a, B: AsMut<[u8]>>(
        ...
        let buffers_state = BuffersState(
            ...
            // dropped before `_backing_buffer` is dropped.
            unsafe { FixedIoBuffer::split_buffer_chunks(buf_slice_mut, self.read_capacity) }
