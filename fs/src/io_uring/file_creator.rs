...
impl<'a> FileCreatorState<'a> {
    ...
    fn mark_write_completed(
        mut ring: RingAccess<'_, '_, Self, FileCreatorOp>,
        file_key: usize,
        fd: types::Fd,
        is_eof_write: bool,
        buf: IoBufferChunk,
...
impl OpenOp {
        ...
        Self: Sized,
    {
        ...
        for (buf, offset, write_len) in backlog {
            if write_len == 0 {
                let ring = RingAccess::Completion(ring);
...
/// Convenience of accessing context and pushing ops on root [Ring] or [Completion]
enum RingAccess<'a, 'b, S, Op>
where
    Op: RingOp<S>,
{
    Ring(&'a mut Ring<S, Op>),
    Completion(&'a mut Completion<'b, S, Op>),
}

impl<'a, 'b, S, Op> RingAccess<'a, 'b, S, Op>
where
    Op: RingOp<S>,
{
    fn context_mut(&mut self) -> &mut S {
        match self {
            Self::Ring(ring) => ring.context_mut(),
            Self::Completion(completion) => completion.context_mut(),
        }
    }

    fn push(&mut self, op: Op) -> io::Result<()> {
        match self {
            Self::Ring(ring) => ring.push(op),
