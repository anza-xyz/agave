...
//!
//! What a shred's bytes mean is `agave-shred-wire-format`; what has to be true of them before they
//! can be trusted is here. Everything is a free function over a view or a hash, so the typestate
//! that decides *when* to call them stays in `agave-shred`.
...
use {
    agave_shred_wire_format::constants, solana_hash::Hash, solana_keypair::Keypair,
    solana_pubkey::Pubkey, solana_signature::Signature, solana_signer::Signer,
