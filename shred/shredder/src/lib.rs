...
//! hand them back as. `agave-shred` wraps [`build_payloads`] into
//! `agave_shred::shredder::FecSet::build`, which reads each payload back through the parser before
//! handing it out, and that is the only place the batch becomes typed shreds.

pub mod error;

pub use agave_shred_wire_format::constants::{CODE_SHREDS, DATA_SHREDS, SHARDS};
use {
...
pub struct FecSetSpec {
    ...
    /// `agave_shred::shred::Shred::check_policy` anyway.
...
pub fn build_payloads(
    ...
    let signature = agave_shred_verify::sign(keypair, tree.root());
    for (index, payload) in payloads.iter_mut().enumerate() {
...
/// `agave-fec-set-recovery`, which reconstructs with the same matrix this encodes with.
