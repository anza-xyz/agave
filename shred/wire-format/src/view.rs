...
//!
//!  This can not be replicated with Wincode traits, since some of the sections returned overlap,
//! and wire format is not quite wincode-compactible.
...
impl<'a, K: ShredLayout> ShredView<'a, K> {
    ...
    fn read_prefix(bytes: &'a [u8]) -> Result<(Self, &'a [u8]), ParseError> {
        ...
        K::check_header(&common, &header, body)?;
