...
/// The part of the header that is common to both shred kinds.
/// The leader's signature occupies bytes `0..64` and is deliberately absent.
/// It is fixed 64 byte field, so it can be a reference into the shred bytes.
...
/// [`Data::check_header`](crate::kind::ShredLayout::check_header) is where a shred that says otherwise is
/// rejected.
