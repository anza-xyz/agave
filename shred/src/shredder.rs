...
//!
//! The batch itself is built by `agave-shredder`, which works in payload bytes and knows nothing
...
pub use agave_shredder::{BatchPosition, CODE_SHREDS, DATA_SHREDS, FecSetSpec, SHARDS, coder};
use {
    crate::{
        error::BuildError,
...
impl FecSet {
    ...
    pub fn build(spec: &FecSetSpec, data: &[u8], keypair: &Keypair) -> Result<Self, BuildError> {
        let (payloads, merkle_root) = agave_shredder::build_payloads(spec, data, keypair)?;
