use {
    ...
    std::{io, net::SocketAddr, num::NonZeroUsize, sync::Arc, time::Duration},
...
const QUIC_HANDSHAKE_TIMEOUT: Duration = Duration::from_millis(2000);
...
/// Number of threads to use for the QUIC runtime sending consensus messages.
const QUIC_RUNTIME_THREADS: usize = 4;

/// Initial CW for QUIC client connection. Deliberately small to avoid making existing congestions worse.
const INITIAL_CONGESTION_WINDOW: u64 = 16 * 1024;
...
impl VotorQuicClient {
    ...
    pub fn new(
        ...
        let endpoint = setup_endpoint(bind, Some(stake_identity), Some(INITIAL_CONGESTION_WINDOW))?;
        let workers = WorkersCache::new(
            NonZeroUsize::new(MAX_ALPENGLOW_VOTE_ACCOUNTS * 2).expect("capacity is nonzero"),
            cancel.clone(),
        );
        ...
        for peer in peers {
                ...
                .try_send_transactions_to_address(&peer, txs_batch.clone())
            {
                ...
                Err(WorkersCacheError::FullChannel) => {
                    warn!("Failed to send consensus message to {peer}: peer not reading messages");
                }
                Err(WorkersCacheError::ReceiverDropped) => {
                    warn!("Failed to send consensus message to {peer}: peer connection refused");
                }
                Err(e) => {
                    warn!("Failed to send consensus message to {peer}: {e:?}");
                }
            }
        }
        drop(tokio_guard);
    ...
    /// Gracefully shuts down all worker connections and waits for them to
    /// finish. Call this before dropping the client to avoid connection errors
    /// when servers are torn down afterwards.
    #[cfg(feature = "dev-context-only-utils")]
    pub fn shutdown(&mut self) {
        self.runtime_handle.block_on(self.workers.shutdown());
    }
    ...
    fn check_for_identity_update(&mut self) {
        ...
        let client_config = build_client_config(
            self.update_identity_receiver.borrow_and_update().as_ref(),
            Some(INITIAL_CONGESTION_WINDOW),
        );
...
mod tests {
    ...
    fn test_quic_identity_update() {
        ...
        let (
            ...
            "quic_client_test",
            [QuicSocket::Kernel(listener_socket)],
            &Keypair::new(),
            sender,
            staked_nodes,
