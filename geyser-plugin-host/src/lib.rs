...
pub struct GeyserPluginHost {
    plugins: Vec<Arc<LoadedGeyserPlugin>>,
}

impl GeyserPluginHost {
    /// Builds a host over an already-loaded set of plugins.
...
pub enum GeyserPluginHostError {
...
pub fn load_plugin_from_config(
...
) -> Result<(LoadedGeyserPlugin, &str), GeyserPluginHostError> {
    type PluginConstructor = unsafe fn() -> *mut dyn GeyserPlugin;

    let mut file = match File::open(geyser_plugin_config_file) {
        Ok(file) => file,
        Err(err) => {
            return Err(GeyserPluginHostError::CannotOpenConfigFile(format!(
    ...
    if let Err(err) = file.read_to_string(&mut contents) {
        return Err(GeyserPluginHostError::CannotReadConfigFile(format!(
    ...
    let result: serde_json::Value = match json5::from_str(&contents) {
        Ok(value) => value,
        Err(err) => {
            return Err(GeyserPluginHostError::InvalidConfigFileFormat(format!(
    ...
    let libpath = result["libpath"]
        .as_str()
        .ok_or(GeyserPluginHostError::LibPathNotSet)?;
    let mut libpath = PathBuf::from(libpath);
    if libpath.is_relative() {
        libpath = geyser_plugin_config_file.with_file_name(libpath);
    }
    ...
    let config_file = geyser_plugin_config_file
        .as_os_str()
        .to_str()
        .ok_or(GeyserPluginHostError::InvalidPluginPath)?;

    let (plugin, lib) = unsafe {
        let lib = Library::new(libpath)
            .map_err(|e| GeyserPluginHostError::PluginLoadError(e.to_string()))?;
...
mod tests {
    ...
    fn enabled_kinds(manager: &GeyserPluginHost) -> [bool; 7] {
        [
    ...
    fn test_push_and_remove_plugin() {
        let mut manager = GeyserPluginHost::default();
    ...
    fn test_from_plugins_preserves_order() {
        let manager = GeyserPluginHost::from_plugins(vec![
    ...
    fn test_notifications_enabled_is_per_kind_and_any_plugin() {
        let manager = GeyserPluginHost::default();
    ...
    fn test_unload_plugin_blocking_waits_for_outstanding_references() {
        ...
        GeyserPluginHost::unload_plugin_blocking(plugin, 0);
        assert!(
    ...
    fn test_load_plugin_from_config_rejects_invalid_json5() {
        ...
        assert!(matches!(
            err,
            GeyserPluginHostError::InvalidConfigFileFormat(_)
        ));
    }
    ...
    fn test_load_plugin_from_config_requires_libpath() {
        ...
        assert!(matches!(err, GeyserPluginHostError::LibPathNotSet));
    }
    ...
    fn test_load_plugin_from_config_resolves_relative_libpath_and_reports_load_errors() {
        ...
        assert!(matches!(err, GeyserPluginHostError::PluginLoadError(_)));
    }
    ...
    fn test_load_plugin_from_config_rejects_missing_file() {
        ...
        assert!(matches!(
            err,
            GeyserPluginHostError::CannotOpenConfigFile(_)
        ));
    }

    #[test]
    fn test_load_plugin_from_config_rejects_non_utf8_contents() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.json");
        std::fs::write(&path, [0xff]).unwrap();
        assert!(matches!(
            load_plugin_from_config(&path),
            Err(GeyserPluginHostError::CannotReadConfigFile(_))
        ));
    }

    #[test]
    fn test_load_shared_library_and_unload_plugin() {
        let library_name = libloading::library_filename("agave_geyser_plugin_host_test_plugin");
        let library = std::env::current_exe()
            .unwrap()
            .parent()
            .unwrap()
            .join(&library_name);
        assert!(library.is_file(), "missing test plugin: {library:?}");

        for relative in [false, true] {
            let dir = tempfile::tempdir().unwrap();
            let libpath = if relative {
                // The test process's working directory differs from the config directory.
                let destination = dir.path().join(&library_name);
                std::fs::copy(&library, destination).unwrap();
            ...
            std::fs::write(
                &config,
                serde_json::json!({ "libpath": libpath, "name": "configured-name" }).to_string(),
            )
            .unwrap();

            let (mut plugin, config_file) = load_plugin_from_config(&config).unwrap();
            assert_eq!(config_file, config.to_str().unwrap());
            assert_eq!(plugin.name(), "configured-name");
            assert_eq!(GeyserPlugin::name(&**plugin), "host-test-plugin");
            assert!(!plugin.transaction_notifications_enabled());
            plugin.on_load(config_file, false).unwrap();
            assert!(plugin.transaction_notifications_enabled());

            let mut manager = GeyserPluginHost::default();
            manager.push_plugin(plugin);
            manager.unload();
            assert!(manager.plugins().is_empty());
            assert_eq!(
                std::fs::read_to_string(config.with_extension("unloaded")).unwrap(),
                "unloaded"
            );
        }
    }
