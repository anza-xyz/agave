...
unsafe impl<C, T> SchemaWrite<C> for LengthPrefixed<T>
where
    C: Config,
    T: SchemaWrite<C, Src = T>,
{
    ...
    fn size_of(src: &Self::Src) -> WriteResult<usize> {
        Ok(size_of::<u16>() + <T as SchemaWrite<C>>::size_of(&src.inner)?)
    }

    fn write(writer: &mut impl Writer, src: &Self::Src) -> WriteResult<()> {
        match Self::TYPE_META {
            TypeMeta::Static { size, .. } => {
                // SAFETY: `size` is the serialized size of the `LengthPrefixed` struct,
                // which is the sum of the serialized size of the `u16` and the inner type.
                //
                // Writing the `u16` length and `T` will fully consume the trusted window.
                let mut writer = unsafe { writer.as_trusted_for(size)? };
                writer.write(&src.len.to_le_bytes())?;
                <T as SchemaWrite<C>>::write(&mut writer, &src.inner)?;
                writer.finish()?;
                Ok(())
            }
            _ => {
                writer.write(&src.len.to_le_bytes())?;
                <T as SchemaWrite<C>>::write(writer, &src.inner)
            }
        }
    }
}

unsafe impl<'de, C, T> SchemaRead<'de, C> for LengthPrefixed<T>
where
    C: Config,
    T: SchemaRead<'de, C, Dst = T>,
{
    ...
    fn read(reader: &mut impl Reader<'de>, dst: &mut MaybeUninit<Self::Dst>) -> ReadResult<()> {
        ...
        let len = unsafe { &mut *(&raw mut (*dst_ptr).len).cast::<MaybeUninit<u16>>() };
        <u16 as SchemaRead<C>>::read(reader, len)?;
        let inner = unsafe { &mut *(&raw mut (*dst_ptr).inner).cast::<MaybeUninit<T>>() };
        <T as SchemaRead<C>>::read(reader, inner)?;
        Ok(())
    }
}
