    ...
    D: TransactionData + Clone,
{
    ...
    let verified_transactions = entry
        ...
        .into_iter()
        .map(|versioned_tx| {
            let num_signers: usize = versioned_tx.num_required_signatures().into();
            let static_account_keys = versioned_tx.static_account_keys();
            ...
            let transaction_view = versioned_tx.clone();
            let verified_transaction = verify(versioned_tx)?;
...
mod tests {
    ...
    /// Serializes `entry` and re-parses it through [`crate::block_component_parser::parse`]
    /// to get a byte-backed [`EntryView`], reusing the same parsing path production uses
