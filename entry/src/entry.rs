...
/// hash was computed by the world's fastest processor at that time. The hash chain is a
/// Proof of Work (not to be confused with Proof of Work consensus!). It is not a Verifiable
/// Delay Function. Verification re-runs every hash rather than checking a succinct proof,
/// so it costs the same total work as generation.
