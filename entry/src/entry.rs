...
impl UnverifiedSignatures {
    ...
    pub fn verify(&self) -> Result<()> {
        self.signatures.par_iter().try_for_each(|tx_signatures| {
            if tx_signatures.signatures.len() != tx_signatures.signer_pubkeys.len() {
...
pub struct HashVerifiedTransactions<Tx: TransactionWithMeta> {
...
fn verify_entry_transaction_hashes<Tx: TransactionWithMeta, F>(
    entry: Entry,
    verify: &F,
...
/// Verifies the hashes of the transactions included in the given entries.
///
...
pub fn verify_transaction_hashes<Tx: TransactionWithMeta + Send + Sync, F>(
    entries: Vec<Entry>,
    num_txs: usize,
    thread_pool: &ThreadPool,
    verify: F,
) -> Result<HashVerifiedTransactions<Tx>>
where
    F: Fn(VersionedTransaction, &[u8]) -> Result<Tx> + Send + Sync,
{
    ...
    if num_txs < PARALLEL_VERIFY_THRESHOLD {
        ...
        let entries = entries
            .into_iter()
            .map(|entry| {
                verify_entry_transaction_hashes(entry, &verify, &mut unverified_signatures)
            })
            .collect::<Result<_>>()?;
        return Ok(HashVerifiedTransactions {
            entries,
...
mod tests {
    ...
    fn test_verify_transactions<Tx: TransactionWithMeta + Send + Sync + 'static>(
        ...
        let hash_verified = verify_transaction_hashes(entries, num_txs, thread_pool, verify);
        let Ok(hash_verified) = hash_verified else {
            return false;
        };
        skip_verification || hash_verified.unverified_signatures.verify().is_ok()
    }
    ...
    #[test]
    fn test_verify_transaction_hashes_and_signatures() {
        let thread_pool = ThreadPoolBuilder::new().build().unwrap();
        let zero = Hash::default();
        let keypair = Keypair::new();
        ...
        let verified_hashes =
            verify_transaction_hashes(entries, 1, &thread_pool, verify_transaction_hash)
                .expect("hash verification must not verify signatures");
        assert_eq!(verified_hashes.entries.len(), 1);
        assert!(verified_hashes.unverified_signatures.verify().is_ok());
        ...
        assert_eq!(verified_hashes.entries.len(), 1);
        assert!(matches!(
            verified_hashes.unverified_signatures.verify(),
