use {
    crate::{
        ip_echo_server::{
            IpEchoServerMessage, IpEchoServerResponse, MAX_PORT_COUNT_PER_MESSAGE,
            SERVER_RESPONSE_PREFIX_LENGTH, SERVER_RESPONSE_TOTAL_LENGTH,
        },
        TIMEOUT,
    },
    anyhow::bail,
...
async fn make_request(
    ...
    let mut bytes = BytesMut::with_capacity(SERVER_RESPONSE_TOTAL_LENGTH);
    bytes.extend_from_slice(&[0u8; SERVER_RESPONSE_PREFIX_LENGTH]);
...
fn parse_response(
    ...
    if response.len() < SERVER_RESPONSE_PREFIX_LENGTH {
        bail!("Response too short, received {} bytes", response.len());
    }

    let (response_header, body) = response
        .split_first_chunk::<SERVER_RESPONSE_PREFIX_LENGTH>()
        .ok_or(anyhow::anyhow!(
            "Not enough data in the response from {ip_echo_server_addr}!"
        ))?;
    let payload = match response_header {
        [0, 0, 0, 0] => bincode::deserialize(
            &response[SERVER_RESPONSE_PREFIX_LENGTH..SERVER_RESPONSE_TOTAL_LENGTH],
        )?,
        [b'H', b'T', b'T', b'P'] => {
