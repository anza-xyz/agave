use {
    ...
    solana_config_interface::{instruction::{self as config_instruction}, state::{get_config_data, ConfigKeys}},
...
fn parse_validator_info(
    ...
    if account.owner != solana_config_interface::program::ID {
...
pub fn process_set_validator_info(
    ...
    let all_config = rpc_client.get_program_accounts(&solana_config_interface::program::ID)?;
    ...
    fn test_parse_validator_info() {
        ...
        assert_eq!(
            parse_validator_info(
                &Pubkey::default(),
                &Account {
                    owner: solana_config_interface::program::ID,
                    data,
                    ..Account::default()
                }
            )
