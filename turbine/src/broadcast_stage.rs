    ...
    let (packets, quic_packets): (Vec<_>, Vec<_>) = shreds
        ...
        .flat_map(|(slot, shreds)| {
            ...
            shreds.flat_map(move |shred| {
                ...
                let maybe_next_leader_info = find_next_leader(slot)
                    .and_then(|leader| {
                        cluster_info.lookup_contact_info(&leader, |node| node.tvu(protocol))
                    })
                    .flatten()
                    .filter(|addr| Some(*addr) != maybe_standard_broadcast_peer);

                [maybe_next_leader_info, maybe_standard_broadcast_peer]
                    .into_iter()
