...
pub(crate) fn get_account_state_growth_delta(
    ...
    let zipped_infos = pre_state_infos
        .iter()
        .zip(post_state_infos.iter())
        .filter_map(|state| {
            if let (Some(pre_state), Some(post_state)) = state {
                Some((pre_state, post_state))
            } else {
                None
            }
        })
        .collect::<Vec<_>>();
    let deleted_accounts: i64 = zipped_infos
        .iter()
        .filter(|(pre_state, post_state)| {
            pre_state.rent_state == RentState::RentExempt
                && post_state.rent_state == RentState::Uninitialized
        })
        .count() as i64;
    let created_accounts: i64 = zipped_infos
        .iter()
        .filter(|(pre_state, post_state)| {
            pre_state.rent_state == RentState::Uninitialized
                && post_state.rent_state == RentState::RentExempt
        })
        .count() as i64;

    zipped_infos
        .iter()
        .map(|(pre_state, post_state)| (pre_state.data_size - post_state.data_size) as i64)
        .sum::<i64>()
        + (created_accounts - deleted_accounts) * solana_rent::ACCOUNT_STORAGE_OVERHEAD as i64
}
