...
/// Collect information about accounts used in txs and
/// return vector of TransactionLoadAccountResult for each transaction in the
/// batch. Each struct contains public keys of accounts used in the transaction
/// and wheather the account was found or not in accounts-db.
...
fn load_transaction_accounts<CB: TransactionProcessingCallback>(
    ...
    let mut accounts: Vec<LoadedAccountDetails> = Vec::with_capacity(account_keys.len());
    ...
    for (account_index, account_key) in account_keys.iter().enumerate() {
        let (account_found, account) = load_transaction_account(
            callbacks,
            message,
            account_key,
            account_index,
        ...
        unique_loaded_accounts.insert(*account_key, account.clone());
        accounts.push(LoadedAccountDetails {
            pubkey: *account_key,
            account_found,
        });
...
fn load_transaction_account<CB: TransactionProcessingCallback>(
        ...
        .and_then(|_| loaded_programs.find(account_key))
    {
        ...
        (account_found, account_shared_data_from_program(&program))
    } else if !unique_loaded_accounts.contains_key(account_key) {
        callbacks
            .get_account_shared_data(account_key)
            .map(|account| (account_found, account))
            .unwrap_or_else(|| {
                account_found = false;
                ...
                (account_found, default_account)
            })
    } else {
        let account = unique_loaded_accounts
            .get(account_key)
            .cloned()
            .unwrap_or_else(|| {
    ...
    let account_state = if account_found {
        AccountState::Alive(&loaded_account.1)
    } else {
        AccountState::Dead
    };
...
mod tests {
    ...
    fn load_accounts_with_features_and_rent(
        ...
        if load_result.is_err() {
            return vec![TransactionLoadResult::NotLoaded(load_result.unwrap_err())];
        } else if program_indices.is_err() {
            return vec![TransactionLoadResult::NotLoaded(
                program_indices.unwrap_err(),
            )];
        }
        ...
        vec![TransactionLoadResult::Loaded(loaded_transaction)]
    }
