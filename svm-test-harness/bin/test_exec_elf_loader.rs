use {
    ...
    solana_svm_test_harness::{
        elf_loader::execute_elf_loader, fixture::proto::ElfLoaderFixture,
    },
    std::path::PathBuf,
};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let Some(effects) = execute_elf_loader(&context) else {
        println!("FAIL: No ELF effects returned for input: {input:?}");
        return false;
    };
