use {
    ...
    solana_svm_test_harness::{
        fixture::proto::SyscallFixture as ProtoSyscallFixture, syscall::fuzz::execute_syscall_proto,
    },
    std::path::PathBuf,
};
...
fn exec(input: &PathBuf) -> bool {
    ...
    let fixture = ProtoSyscallFixture::decode(&blob[..]).unwrap();
    ...
    let Some(effects) = execute_syscall_proto(context) else {
        println!("FAIL: No syscall effects returned for input: {input:?}",);
        return false;
    };
