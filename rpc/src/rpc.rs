...
fn new_response<T>(bank: &Bank, value: T) -> RpcResponse<T> {
    RpcResponse {
        context: RpcResponseContext::new(bank.slot(), Some(bank.block_height())),
        value,
    }
}
...
impl JsonRpcRequestProcessor {
    ...
    async fn get_largest_accounts(
        ...
        if let Some((slot, accounts)) = self.get_cached_largest_accounts(&config.filter) {
            Ok(RpcResponse {
                context: RpcResponseContext::new(slot, Some(bank.block_height())),
                value: accounts,
            })
        } else {
...
pub mod tests {
    ...
    fn test_rpc_get_program_accounts() {
        ...
        let expected = RpcResponse {
            context: RpcResponseContext::new(0, Some(0)),
