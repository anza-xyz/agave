...
fn recover_secp256k1_pubkey_legacy(
    hash: &[u8],
    recovery_id_val: u64,
    signature: &[u8],
) -> Result<[u8; 65], Secp256k1RecoverError> {
    let message =
        libsecp256k1::Message::parse_slice(hash).map_err(|_| Secp256k1RecoverError::InvalidHash)?;
    let adjusted_recovery_id = recovery_id_val
        .try_into()
        .map_err(|_| Secp256k1RecoverError::InvalidRecoveryId)?;
    let recovery_id = libsecp256k1::RecoveryId::parse(adjusted_recovery_id)
        .map_err(|_| Secp256k1RecoverError::InvalidRecoveryId)?;
...
fn recover_secp256k1_pubkey_k256(
    hash: &[u8],
    recovery_id_val: u64,
    signature: &[u8],
) -> Result<[u8; 65], Secp256k1RecoverError> {
    if hash.len() != keccak::HASH_BYTES {
    ...
    let public_key = k256::ecdsa::VerifyingKey::recover_from_prehash(hash, &signature, recovery_id)
        .map_err(|_| Secp256k1RecoverError::InvalidSignature)?;
    let mut serialized = [0u8; 65];
    serialized.copy_from_slice(public_key.to_encoded_point(false).as_bytes());
    Ok(serialized)
}
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        let public_key = match invoke_context.get_feature_set().secp256k1_use_k256 {
            true => recover_secp256k1_pubkey_k256(hash, recovery_id_val, signature),
            false => recover_secp256k1_pubkey_legacy(hash, recovery_id_val, signature),
        };
...
mod tests {
    ...
    fn test_syscall_secp256k1_recover() {
        ...
        let expected_pubkey: [u8; SECP256K1_PUBLIC_KEY_LENGTH] = [
        ...
        let signature: [u8; SECP256K1_SIGNATURE_LENGTH] = [
        ...
        let mut recovered_pubkey = [0u8; SECP256K1_PUBLIC_KEY_LENGTH];
        let mut memory_mapping = MemoryMapping::new(
            vec![
                MemoryRegion::new_readonly(&hash, hash_va),
                MemoryRegion::new_readonly(&signature, signature_va),
                MemoryRegion::new_writable(&mut recovered_pubkey, result_va),
            ],
            &config,
            SBPFVersion::V3,
        )
        ...
        let cost = invoke_context.get_execution_cost().secp256k1_recover_cost;
        invoke_context.mock_set_remaining(2 * cost);
    ...
    #[test]
    fn test_syscall_secp256k1_recover_allows_malleable_signatures() {
        let config = Config::default();
        prepare_mockup!(invoke_context, program_id, bpf_loader::id());
        ...
        let signature_bytes: [u8; SECP256K1_SIGNATURE_LENGTH] = [
        ...
        let alt_recovery_id = libsecp256k1::RecoveryId::parse(1).unwrap().serialize();

        let hash_va = 0x100000000;
        let signature_va = 0x200000000;
        let alt_signature_va = 0x300000000;
        let result_va = 0x400000000;
        let alt_result_va = 0x500000000;
        let mut recovered_pubkey = [0u8; SECP256K1_PUBLIC_KEY_LENGTH];
        let mut alt_recovered_pubkey = [0u8; SECP256K1_PUBLIC_KEY_LENGTH];
        let mut memory_mapping = MemoryMapping::new(
            vec![
                MemoryRegion::new_readonly(message_hash.as_ref(), hash_va),
                MemoryRegion::new_readonly(&signature_bytes, signature_va),
                MemoryRegion::new_readonly(&alt_signature_bytes, alt_signature_va),
                MemoryRegion::new_writable(&mut recovered_pubkey, result_va),
                MemoryRegion::new_writable(&mut alt_recovered_pubkey, alt_result_va),
            ],
            &config,
            SBPFVersion::V3,
        )
        ...
        let cost = invoke_context.get_execution_cost().secp256k1_recover_cost;
        invoke_context.mock_set_remaining(3 * cost);
    ...
    #[test]
    fn test_syscall_secp256k1_recover_with_k256_gate_allows_malleable_signatures() {
        let config = Config::default();
        let feature_set = SVMFeatureSet {
            secp256k1_use_k256: true,
            ..Default::default()
        };
