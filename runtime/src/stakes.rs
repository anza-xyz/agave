...
impl StakesCache {
    ...
    pub(crate) fn remove_inert_stake_delegations(&self, new_rate_activation_epoch: Option<Epoch>) {
        let mut stakes = self.0.write().unwrap();
        stakes.remove_inert_stake_delegations(new_rate_activation_epoch);
    }
}
...
impl Stakes<StakeAccount> {
    ...
    fn remove_inert_stake_delegations(&mut self, new_rate_activation_epoch: Option<Epoch>) {
...
pub(crate) mod tests {
    ...
    fn test_inert_delegation_is_pruned_on_load(case: InertTestState) {
        ...
        stakes_cache.check_and_store(&vote_pubkey, &vote_account, None, false, false);
        stakes_cache.check_and_store(&stake_pubkey, &stake_account, None, false, false);
        stakes_cache.remove_inert_stake_delegations(None);
