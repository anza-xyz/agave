...
impl Bank {
    /// A scheduler-level single-transaction check that returns the nonce address
    /// for nonce map filtering and does not check the status cache.
    pub fn precheck_transaction_on_ingest(
        &self,
    ...
    /// The critical (consensus) check function that runs before SVM in both block-production
    /// and replay. All other `check_transactions_internal()` are leader-only wrappers that run
    /// outside of the batch execution context and can provide different behavior or ergonomics.
    pub(super) fn check_transactions_before_execution<Tx: TransactionWithMeta>(
        ...
        self.check_transactions_internal(txs, lock_results, max_age, false, false, error_counters)
            .0
    }

    /// A general-purpose scheduler-level check over held transactions.
    pub fn check_transactions_for_scheduling<Tx: TransactionWithMeta>(
        &self,
        txs: &[impl core::borrow::Borrow<Tx>],
        lock_results: &[TransactionResult<()>],
        max_age: usize,
        ...
        self.check_transactions_internal(txs, lock_results, max_age, false, true, error_counters)
            .0
    }

    /// A scheduler-level check that returns processed slots for `AlreadyProcessed` transactions.
    pub fn check_transactions_with_processed_slots<Tx: TransactionWithMeta>(
        ...
        self.check_transactions_internal(txs, lock_results, max_age, true, true, error_counters)
    }

    /// A scheduler-level check that truncates `max_age` to avoid forwarding soon-to-expire transactions.
    pub fn check_transactions_with_forwarding_delay(
        ...
        let notional_max_age = self
            .max_processing_age()
            .saturating_sub(max_tx_fwd_delay)
            .saturating_sub(forward_transactions_to_leader_at_slot_offset as usize);

        self.check_transactions_internal(
            txs,
            lock_results,
            notional_max_age,
            false,
    ...
    fn check_transactions_internal<Tx: TransactionWithMeta>(
        &self,
        txs: &[impl core::borrow::Borrow<Tx>],
        lock_results: &[TransactionResult<()>],
        max_age: usize,
...
mod tests {
    ...
    fn test_check_transaction_without_status_cache_allows_already_processed() {
        ...
        let (check_results, _) = bank.check_transactions_with_processed_slots(
        ...
        let mut error_counters = TransactionErrorMetrics::default();
        let check_result = bank.precheck_transaction_on_ingest(
            &tx,
            bank.max_processing_age(),
            &mut error_counters,
        );
