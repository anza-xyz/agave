...
fn sweep_vote_account(
    ...
    if stake != 0 && pending_delegator_rewards != 0 {
        account
...
impl Bank {
    ...
    pub(in crate::bank) fn distribute_reward_commissions(
        ...
        let (epoch_boundary_accounts, load_and_update_epoch_boundary_accounts_us) = measure_us!(
            self.load_and_update_epoch_boundary_accounts(reward_commissions, thread_pool)
        );
    ...
    fn load_and_update_epoch_boundary_accounts(
        ...
        let stakes = self.stakes_cache.stakes();
        let (commission_receiving_vote_stakes, other_vote_stakes): (HashMap<_, _>, HashMap<_, _>) =
            stakes
                .vote_accounts()
                .delegated_stakes()
                .partition(|(vote_address, _)| {
...
mod tests {
    ...
    fn test_load_and_reward_commission_accounts_empty() {
        ...
        let result =
            bank.load_and_update_epoch_boundary_accounts(&reward_commissions, &thread_pool);
