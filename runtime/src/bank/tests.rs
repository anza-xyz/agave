...
fn test_bank_capitalization() {
    ...
    let bank1 = Bank::new_from_parent(bank0, BankLeader::default(), 1);
    assert_eq!(
...
pub(in crate::bank) fn new_from_parent_next_epoch(
    ...
    Bank::new_from_parent_with_bank_forks(bank_forks, parent, BankLeader::default(), slot)
}
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let bank2 = Bank::new_from_parent(bank1.clone(), BankLeader::default(), bank1.slot() + 1);
...
fn test_readonly_accounts(relax_intrabatch_account_locks: bool) {
    ...
    let bank = Bank::new_from_parent(Arc::new(bank), BankLeader::default(), next_slot);
...
fn new_from_parent_with_fork_next_slot(parent: Arc<Bank>, fork: &RwLock<BankForks>) -> Arc<Bank> {
    let slot = parent.slot() + 1;
    Bank::new_from_parent_with_bank_forks(fork, parent, BankLeader::default(), slot)
}
...
fn test_bank_get_account_in_parent_after_squash2() {
    ...
    bank5.squash();
    let bank6 =
        Bank::new_from_parent_with_bank_forks(bank_forks.as_ref(), bank5, BankLeader::default(), 6);
    bank6.squash();
...
fn test_bank_get_account_modified_since_parent_with_fixed_root() {
    ...
    let bank3 =
        Bank::new_from_parent_with_bank_forks(bank_forks.as_ref(), bank2, BankLeader::default(), 3);
    assert_eq!(
        None,
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (bank, _bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    bank.squash();
    let bank = Bank::new_from_parent(bank, BankLeader::new_unique(), 1);
...
fn test_add_duplicate_static_program() {
    ...
    let mut bank = Bank::new_from_parent(bank, BankLeader::default(), slot);
...
fn test_clean_nonrooted() {
    ...
    let bank2 = Arc::new(Bank::new_from_parent(bank0, BankLeader::default(), 2));
...
fn test_add_builtin_account_after_frozen() {
    ...
    let (parent_bank, _bank_forks) = create_simple_test_arc_bank(100_000);
    let bank = Bank::new_from_parent(parent_bank, BankLeader::default(), slot);
    bank.freeze();
...
fn test_bank_load_program() {
    ...
    goto_end_of_slot(bank.clone());
    let bank = Bank::new_from_parent_with_bank_forks(&bank_forks, bank, BankLeader::default(), 42);
    let bank = Bank::new_from_parent_with_bank_forks(&bank_forks, bank, BankLeader::default(), 50);
...
fn test_bpf_loader_upgradeable_deploy_with_max_len() {
    ...
    let bank = bank_client
        .advance_slot(1, bank_forks.as_ref(), BankLeader::default())
        .unwrap();
...
fn test_block_limits() {
    ...
    let mut bank = Bank::new_from_parent(Arc::new(bank), BankLeader::default(), 2);
    assert_eq!(
        bank.read_cost_tracker().unwrap().get_block_limit(),
        MAX_BLOCK_UNITS_SIMD_0286,
...
fn test_check_reserved_keys() {
    let (genesis_config, _mint_keypair) = create_genesis_config(1);
    let bank = Bank::new_for_tests(&genesis_config);
    let mut bank = Bank::new_from_parent(Arc::new(bank), BankLeader::new_unique(), 1);
...
fn test_feature_activation_loaded_programs_cache_preparation_phase() {
    ...
    goto_end_of_slot(bank.clone());
    let bank = Bank::new_from_parent_with_bank_forks(&bank_forks, bank, BankLeader::default(), 16);
    ...
    goto_end_of_slot(bank.clone());
    let bank = Bank::new_from_parent_with_bank_forks(&bank_forks, bank, BankLeader::default(), 33);
...
fn test_feature_activation_loaded_programs_epoch_transition() {
    ...
    goto_end_of_slot(bank.clone());
    let bank = Bank::new_from_parent_with_bank_forks(&bank_forks, bank, BankLeader::default(), 31);

    // Advance the bank to cross the epoch boundary and activate the feature.
    goto_end_of_slot(bank.clone());
    let bank = Bank::new_from_parent_with_bank_forks(&bank_forks, bank, BankLeader::default(), 32);
...
fn test_register_hard_fork() {
    ...
    let bank7 = Bank::new_from_parent(bank0.clone(), BankLeader::default(), 7);
    ...
    let bank9 = Bank::new_from_parent(bank0, BankLeader::default(), 9);
    bank9.freeze();
...
fn test_last_restart_slot() {
    ...
    let mut bank1 = Arc::new(Bank::new_from_parent(bank0, BankLeader::default(), 1));
    ...
    let bank3 = Arc::new(Bank::new_from_parent(bank1, BankLeader::default(), 3));
    ...
    let bank4 = Arc::new(Bank::new_from_parent(bank2, BankLeader::default(), 4));
    assert!(!last_restart_slot_dirty(&bank4));
    assert_eq!(get_last_restart_slot(&bank4), Some(0));
    let bank5 = Arc::new(Bank::new_from_parent(bank3, BankLeader::default(), 5));
    ...
    let bank6 = Arc::new(Bank::new_from_parent(bank4, BankLeader::default(), 6));
    ...
    let bank7 = Arc::new(Bank::new_from_parent(bank6, BankLeader::default(), 7));
...
fn test_parent_block_id() {
    ...
    // expected value.
    let child_bank = Bank::new_from_parent(parent_bank, BankLeader::new_unique(), 1);
