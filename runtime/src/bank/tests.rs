...
fn test_invalid_rent_state_changes_existing_accounts() {
    ...
    // Existing accounts are considered rent-exempt by default and not allowed to
    // become RentPaying; debiting below pre-balance should fail if the pre-execution
    // balance is below the current rent-exempt minimum.
    ...
    let result = bank.process_transaction(&tx);
    assert!(result.is_err());
    assert!(!check_account_is_rent_exempt(&test_account.pubkey()));
    let tx = create_mock_transfer(
        &mint_keypair, // payer
        &test_account, // from
        &mint_keypair, // to
    ...
    let result = bank.process_transaction(&tx);
    assert!(result.is_ok());
    assert!(bank.get_account(&test_account.pubkey()).is_none());
    ...
    let result = bank.transfer(1, &mint_keypair, &test_account.pubkey());
    assert!(result.is_ok());
    assert!(check_account_is_rent_exempt(&test_account.pubkey()));
