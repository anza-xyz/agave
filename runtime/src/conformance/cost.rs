...
use {
    super::build_versioned_message,
    protosol::protos::{
        CostContext, CostResult, SanitizedTransaction as ProtoSanitizedTransaction, TxnCostMode,
    },
...
fn runtime_transaction_from_proto(
    ...
    let serialized_size = bincode::serialized_size(&versioned_tx).ok()?;
...
fn result_from_transaction_cost<Tx>(cost: &TransactionCost<'_, Tx>) -> CostResult
where
...
pub fn execute_cost(input: &CostContext) -> CostResult {
    let proto_tx = input.tx.as_ref().expect("CostContext missing transaction");
    let runtime_tx = runtime_transaction_from_proto(proto_tx)
        .expect("failed to build RuntimeTransaction from proto");
    ...
    let cost = if input.mode == TxnCostMode::Actual as i32 {
    ...
    result_from_transaction_cost(&cost)
}
...
pub unsafe extern "C" fn sol_compat_txn_cost_v1(
    ...
    let Ok(cost_context) = CostContext::decode(in_slice) else {
        return 0;
    };
...
mod tests {
    use {
        ...
        protosol::protos::{
            CompiledInstruction as ProtoCompiledInstruction, CostContext,
            FeatureSet as ProtoFeatureSet, MessageHeader as ProtoMessageHeader,
            SanitizedTransaction as ProtoSanitizedTransaction,
            TransactionMessage as ProtoTransactionMessage, TxnCostMode,
        },
    ...
    fn estimate_context(tx: ProtoSanitizedTransaction) -> CostContext {
        CostContext {
            tx: Some(tx),
            features: Some(all_features()),
            mode: TxnCostMode::Estimate as i32,
            actual_programs_execution_cost: 0,
            actual_loaded_accounts_data_size_bytes: 0,
        }
    }

    fn assert_has_cost(ctx: &CostContext) -> CostResult {
    ...
    fn test_actual_mode_uses_provided_costs() {
        ...
        let ctx = CostContext {
            ...
            features: Some(all_features()),
            mode: TxnCostMode::Actual as i32,
    ...
    #[test]
    #[should_panic(expected = "CostContext missing transaction")]
    ...
    #[test]
    #[should_panic(expected = "failed to build RuntimeTransaction from proto")]
