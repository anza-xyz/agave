...
impl PohRecorder {
    ...
    // Compute target tick duration in nanoseconds based on the bank nanos per
    // slot and ticks per slot. Working bank is used when we are leader and
    // start bank is used when we are not.
