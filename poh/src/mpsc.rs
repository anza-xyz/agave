...
//! Usage
//! ```
//! // Create channel ends with a fixed capacity of 4 and slot: 10
//! let bank_id = 10;
//! let (tx, rx) = solana_poh::mpsc::with_capacity(4, bank_id);
//!
...
pub fn with_capacity<T>(capacity: u32, bank_id: u64) -> (Producer<T>, Consumer<T>) {
    let ring_buffer = std::sync::Arc::new(ring_buffer::RingBuffer::with_capacity(capacity, bank_id));
...
mod tests {
    ...
    fn example() {
        ...
        let bank_id = 100;
        let (tx, rx) = crate::mpsc::with_capacity(4, bank_id);
