...
impl PohService {
    ...
    pub fn target_ns_per_tick(ticks_per_slot: u64, target_tick_duration_ns: u64) -> u64 {
        let target_slot_duration_ns = target_tick_duration_ns.saturating_mul(ticks_per_slot);
        let target_slot_adjustment_ns = TARGET_SLOT_ADJUSTMENT_NS.min(
            // Preserve the existing default ratio: adjust by at most 12.5% of
            // the target slot duration.
            target_slot_duration_ns.saturating_div(8),
        );
