...
pub(crate) struct QuicDatagramStats {
    ...
    /// Validated inbound refused before the handshake: its source IP is not a
    /// gossip-advertised address of any staked peer.
    pub(crate) handshake_rejected_unknown_ip: AtomicU64,
    /// Inbound refused before the handshake: the source IP already had a
    /// handshake accepted within the last `MAX_IDLE_TIMEOUT` (one in-flight
    /// handshake per IP).
    pub(crate) handshake_rejected_inflight: AtomicU64,
}
...
pub(crate) fn report(stats: &QuicDatagramStats, live_connections: u64) {
    ...
    datapoint_info!(
        ...
        (
            "handshake_rejected_unknown_ip",
            swap!(stats.handshake_rejected_unknown_ip),
            i64
        ),
        (
            "handshake_rejected_inflight",
            swap!(stats.handshake_rejected_inflight),
            i64
        ),
        (
