...
mod tests {
    ...
    fn test_access_violation_handler() {
        ...
        // Reading beyond writable accounts current size grows it to 4 KiB boundary
        memory_mapping
        ...
        // Writing beyond writable accounts current size grows it to 8 KiB boundary
        memory_mapping
            .store::<u32>(0, account_start_offsets[1] + 0x1000)
            .unwrap();
        assert_eq!(
            transaction_context
                .accounts()
        ...
        // Writing beyond writable accounts current size again only grows it
        // to original length plus MAX_PERMITTED_DATA_INCREASE (which is less than the 12 KiB boundary)
        memory_mapping
            .store::<u32>(0, account_start_offsets[1] + 0x2000)
            .unwrap();
        assert_eq!(
            transaction_context
                .accounts()
        ...
        // Accessing the rest of the address space reserved for the almost max sized writable account is denied
        memory_mapping
        ...
        // Writing beyond almost writable accounts current size
        // only grows it to MAX_PERMITTED_ACCOUNTS_DATA_ALLOCATIONS_PER_TRANSACTION
        memory_mapping
