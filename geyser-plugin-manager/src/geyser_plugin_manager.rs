...
#[cfg(test)]
use agave_geyser_plugin_host::{GeyserPluginHostError, LoadedGeyserPlugin};
use {
...
pub(crate) fn list_plugins(manager: &GeyserPluginHost) -> JsonRpcResult<Vec<String>> {
    Ok(manager
        .plugins()
        .iter()
...
pub(crate) fn unload_plugin(
    ...
    let mut new_plugin_manager: GeyserPluginHost = (*plugin_manager.load_full()).clone();
    ...
    plugin_manager.store(Arc::new(new_plugin_manager));
    GeyserPluginHost::unload_plugin_blocking(plugin_ref, idx);
...
/// Returns a new instance of GeyserPluginHost
...
#[cfg(test)]
pub(crate) const TESTPLUGIN_CONFIG: &str = "TESTPLUGIN_CONFIG";
#[cfg(test)]
pub(crate) const TESTPLUGIN2_CONFIG: &str = "TESTPLUGIN2_CONFIG";
#[cfg(test)]
const FAILING_TESTPLUGIN_CONFIG: &str = "FAILING_TESTPLUGIN_CONFIG";
...
pub(crate) fn load_plugin_from_config(
    ...
    if geyser_plugin_config_file.ends_with(FAILING_TESTPLUGIN_CONFIG) {
        Ok(tests::dummy_plugin_and_library(
            tests::FailingTestPlugin,
            FAILING_TESTPLUGIN_CONFIG,
        ))
    } else if geyser_plugin_config_file.ends_with(TESTPLUGIN_CONFIG) {
        Ok(tests::dummy_plugin_and_library(
        ...
        Err(GeyserPluginHostError::CannotOpenConfigFile(
...
mod tests {
    use {
        crate::{
            ...
            geyser_plugin_manager::{
                FAILING_TESTPLUGIN_CONFIG, TESTPLUGIN_CONFIG, TESTPLUGIN2_CONFIG, list_plugins,
                load_plugin, reload_plugin, unload_plugin,
            },
    ...
    #[derive(Debug)]
    pub(super) struct FailingTestPlugin;

    impl GeyserPlugin for FailingTestPlugin {
        fn name(&self) -> &'static str {
            "fails-on-load"
        }
        ...
        fn on_load(&mut self, _config_file: &str, _is_reload: bool) -> PluginResult<()> {
            Err(
                agave_geyser_plugin_interface::geyser_plugin_interface::GeyserPluginError::Custom(
                    Box::new(std::io::Error::other("test initialization failure")),
                ),
            )
        }
    }
    ...
    fn test_geyser_reload() {
        ...
        let plugin_manager = Arc::new(ArcSwap::new(Arc::new(GeyserPluginHost::default())));
    ...
    fn test_plugin_list() {
        // Initialize empty manager
        let plugin_manager = Arc::new(RwLock::new(GeyserPluginHost::default()));
    ...
    #[test]
    fn test_plugin_start_failure_keeps_manager_empty() {
        let manager = ArcSwap::from_pointee(GeyserPluginHost::default());
        let error = load_plugin(&manager, FAILING_TESTPLUGIN_CONFIG).unwrap_err();
        assert_eq!(error.code, jsonrpc_core::ErrorCode::InvalidRequest);
        assert!(error.message.contains("test initialization failure"));
        assert!(manager.load().plugins().is_empty());

        load_plugin(&manager, TESTPLUGIN_CONFIG).unwrap();
        let error = reload_plugin(&manager, DUMMY_NAME, FAILING_TESTPLUGIN_CONFIG).unwrap_err();
        assert_eq!(error.code, jsonrpc_core::ErrorCode::InvalidRequest);
        assert!(error.message.contains("previous plugin was dropped"));
        assert!(error.message.contains("test initialization failure"));
        assert!(manager.load().plugins().is_empty());
    }
    ...
    fn test_deshred_transaction_notifications_enabled() {
        let empty_manager = GeyserPluginHost::default();
        assert!(!empty_manager.deshred_transaction_notifications_enabled());

        let disabled_manager = GeyserPluginHost::from_plugins(vec![Arc::new(
        ...
        let enabled_manager = GeyserPluginHost::from_plugins(vec![Arc::new(
            dummy_plugin_and_library(
                DeshredTestPlugin {
    ...
    fn test_deshred_transaction_notifier_forwards_only_enabled_plugins() {
        ...
        let plugin_manager = Arc::new(ArcSwap::new(Arc::new(GeyserPluginHost::from_plugins(
            vec![
                Arc::new(
                    dummy_plugin_and_library(
                        DeshredTestPlugin {
